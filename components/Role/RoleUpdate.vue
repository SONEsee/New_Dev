...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Navigate to the role detail page or back to list
    await navigateTo(`/roles/${response.role_id}`)
...
// Auto-format Role ID as user types
watch(() => form.role_id, (newValue) => {
  // Convert to uppercase and replace invalid characters
  form.role_id = newValue.toUpperCase().replace(/[^A-Z0-9_]/g, '')
})
</script>
